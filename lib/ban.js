...
module.exports.run = async (client, message, args, conn) => {
  ...
  const reason = args[1];
  if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send(new Discord.RichEmbed().setAuthor("Ошибка! У вас нет прав!").setColor("RED").setTimestamp());
  if (member.id == message.author.id) return message.channel.send(new Discord.RichEmbed().setAuthor("Ошибка! Вы не можете забанить самого себя!").setColor("RED").setTimestamp());
