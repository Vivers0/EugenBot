...
module.exports.run = async (client, message, args) => {
    ...
    if(!song) return message.channel.send(new Discord.RichEmbed().setAuthor("Ошибка! Вы не указали ссылку на трек!").setColor("RED"));
    if(!voice) return message.channel.send(new Discord.RichEmbed().setAuthor("Ошибка! Вы не вошли в голосовой канал!").setColor("RED"));

    // let valid = ytdl.validateID(song);
    // if(!valid) return message.channel.send(new Discord.RichEmbed().setAuthor("Ошибка! Ссылка недействительна!").setColor("ORANGE"));
    ...
    message.channel.send(new Discord.RichEmbed().setAuthor("Трек добавлен в очередь!").setColor("GREEN"));
