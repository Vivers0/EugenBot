...
module.exports.run = async (client, message, args) => {
    ...
    if(!song) return message.channel.send(new MessageEmbed().setAuthor("Ошибка! Вы не указали ссылку на трек!").setColor("RED"));
    if(!voice) return message.channel.send(new MessageEmbed().setAuthor("Ошибка! Вы не вошли в голосовой канал!").setColor("RED"));
    ...
    message.channel.send(new MessageEmbed().setAuthor("Трек добавлен в очередь!").setColor("GREEN"));
