...
module.exports.run = async (client, message, args, conn) => {
  ...
  if (!args[0])
    return message.channel.send(
      new RichEmbed().setAuthor("Вы не указали участника!")
    );
  if (!rUser)
    return message.channel.send(
      new RichEmbed()
  ...
  if (!args[1])
    return message.channel.send(
      new RichEmbed().setAuthor("Укажите время!")
    );
  let rreason = args.slice(2).join(" ");
  if (!rreason)
    return message.channel.send(
      new RichEmbed().setAuthor("Укажите причину!")
    );
  ...
  conn.query(
    ...
    (error, rows, fields) => {
      ...
      if (rUser.roles.has(role.id) | rows.length > 1)
        return message.channel.send(
          new RichEmbed().setAuthor("пользователь уже в муте!")
        );
      else {
        conn.query(
