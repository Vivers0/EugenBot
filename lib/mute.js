...
module.exports.run = async (client, message, args, conn) => {
  ...
  if (!args[0])
    return message.channel.send(
      new Discord.RichEmbed().setAuthor("Вы не указали участника!")
    );
  if (!rUser)
    return message.channel.send(
  ...
  if (!args[1])
    return message.channel.send(
      new Discord.RichEmbed().setAuthor("Укажите время!")
    );
  let rreason = args.slice(2).join(" ");
  if (!rreason)
    return message.channel.send(
      new Discord.RichEmbed().setAuthor("Укажите причину!")
    );
  ...
  conn.query(
    ...
    (error, rows, fields) => {
      ...
      if (rUser.roles.has(role.id) | rows.length > 1)
        return message.channel.send(
          new Discord.RichEmbed().setAuthor("пользователь уже в муте!")
        );
      else {
        conn.query(
          `INSERT INTO mutes (guild, user_id, time) VALUES (${
            message.guild.id
          }, ${rUser.id}, ${parseInt(Date.now() + mutetime)})`
        );
        if (error) console.log(error);
        rUser.addRole(role).then(() => {
