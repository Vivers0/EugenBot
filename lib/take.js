...
module.exports.run = async (client, message, args, conn) => {
    ...
    conn.query('SELECT * FROM account WHERE d_id = ?', [message.author.id], (err, rows) => {
        ...
        if (mon < money) {
            return message.channel.send(new MessageEmbed().setDescription("У вас не хватает денег!").setColor("ORANGE"));
        } else {
            ...
            conn.query('SELECT * FROM account WHERE d_id = ?', [rUser.id], (err, rows) => {
                ...
                message.channel.send(new MessageEmbed().setDescription(`${message.author.tag} передал ${rUser} \`${money}\``).setColor("GREEN"));
            });
        }
    });
}
