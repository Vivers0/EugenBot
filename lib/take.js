...
conn.query('SELECT * FROM account WHERE d_id = ?', [message.author.id], (err, rows) => {
    ...
    if(mon < money){
        return message.channel.send(new RichEmbed().setDescription("У вас не хватает денег!").setColor("ORANGE"));
    } else {
    ...
    let mon = Number(rows[0].money);
        ...
        message.channel.send(new RichEmbed().setDescription(`${message.author.tag} передал ${rUser} \`${money}\``).setColor("GREEN"));
})
}
});
